...
(**
    ...
    evaluated expression and the time it took to run
*)
...
(**
    ...
    given input, and returns the function result and the time it took to run; it may be useful to only provide the first
    argument so that you can time the same function with various different inputs
*)
...
(**
    ...
    input, and returns the time it took to run; it may be useful to only provide the first argument so that you can time
    the same function with various different inputs
*)
